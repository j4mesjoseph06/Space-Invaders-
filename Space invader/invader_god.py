...
def Win_text(x, y):
    win_text = over_font.render("YOU WIN!", True, color_light)
    screen.blit(win_text, (350, 120))

def winner_text(x, y):
    a = xsmallfont.render("CONGRATULATIONS, YOU HAVE MANAGED TO BEAT GOD TIER!", True, color_light)
    screen.blit(a, (50, 200))
    d = xsmallfont.render("THANK YOU FOR PLAYING MY GAME, I HOPE YOU ENJOYED IT!", True, color_light)
    screen.blit(d, (52, 230))
...
#boundaries and movement of enemy
    for i in range(num_of_enemies):
        ...
        #game over
        if enemyY[i] >= 460 and score_value != 70:
            ...
            winner_text(textX, textY)
            mouse = pygame.mouse.get_pos()
            if event.type == pygame.MOUSEBUTTONDOWN:
                if width/2-40 <= mouse[0] <= width/2+90 and height/1.9 <= mouse[1] <= height/1.9+40:
                    pygame.quit()
                if width/2-115 <= mouse[0] <= width/2+90 and height/1.65 <= mouse[1] <= height/1.65+40:
                    two()
            ...
            mouse = pygame.mouse.get_pos()
            if width/2-40 <= mouse[0] <= width/2+90 and height/1.9 <= mouse[1] <= height/1.9+40:
                pygame.draw.rect(screen,color_light,[width/2-40,height/1.9,80,40])
                screen.blit(quit_text ,(width/2-40,height/1.9+3))
            else:
                pygame.draw.rect(screen,color_dark,[width/2-40,height/1.9,80,40])
                screen.blit(quit_text ,(width/2-40,height/1.9+3))
